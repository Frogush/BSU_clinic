...
function createCard(item) {
    ...
    // Используем item.image_url и item.content (который уже массив)
    article.innerHTML = `
        <div class="news-card__image-box">
            <img class="news-card__image" src="${item.image_url}" alt="${item.title}">
        </div>
...
function renderNextBatch() {
    ...
    nextBatch.forEach(item => newsGrid.appendChild(createCard(item)));
    currentIndex += ITEMS_TO_SHOW;

    if (currentIndex >= newsData.length && moreBtn) {
